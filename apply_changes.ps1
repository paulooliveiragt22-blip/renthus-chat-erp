...
# Exemplo: PS C:\disk-bebidas\renthus-chat-erp> .\apply_changes.ps1

# --- Configurações
$branchName = "feat/ui-sidebar-product-modal"
$filesToBackup = @(
    "styles/globals.css",
    "app/layout.tsx",
...
foreach ($f in $filesToBackup) {
    if (Test-Path $f) {
        ...
        Write-Host "Backup criado: $f.bak"
    } else {
        Write-Host "Arquivo não existe (será criado): $f"
    }
}
...
/* Colocar regras base dentro de @layer base para garantir que @apply funcione */
@layer base {
  html, body {
    min-height: 100%;
    /* define a cor de fundo diretamente para evitar erros de utilitário durante o build */
    background-color: #f2f2f4;
  }

  /* Ajustes globais adicionais (opcionais) */
  body {
    margin: 0;
...
/* Card padrão: fundo branco e sombra para sensação de profundidade */
.card {
  background: #fff;
  border-radius: 12px;
  border: 1px solid #eee;
  box-shadow: 0 8px 20px rgba(32,26,43,0.06);
}
'@

$globals | Out-File -FilePath "styles/globals.css" -Encoding utf8
Write-Host "Wrote styles/globals.css"
...
$layout | Out-File -FilePath "app/layout.tsx" -Encoding utf8
...
// Formulário de perfil (simples, integrado)
function CompanyProfileForm({ company, onChange, onLoad, onSave }: { company: any; onChange: (c: any) => void; onLoad: () => Promise<void>; onSave: () => Promise<void> }) {
    ...
    async function handleCepLookup() {
        ...
        try {
            ...
            if (json.erro) return alert("CEP não encontrado");
    ...
    return (
        <div style={{ display: "grid", gap: 8 }}>
            <input value={company?.company_name ?? ""} onChange={(e) => updateField("company_name", e.target.value)} placeholder="Razão social" />
            ...
            <input value={company?.number ?? ""} onChange={(e) => updateField("number", e.target.value)} placeholder="Número (manual)" />
            ...
            <input value={company?.responsible ?? ""} onChange={(e) => updateField("responsible", e.target.value)} placeholder="Responsável" />

            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                <button onClick={onSave} style={{ background: "#3B246B", color: "#fff", padding: "8px 12px", borderRadius: 10 }}>Salvar</button>
                <button onClick={() => { /* fechar é responsabilidade do pai */ }} style={{ padding: "8px 12px", borderRadius: 10 }}>Cancelar</button>
            </div>
        </div>
    );
}
...
export default function AdminSidebar() {
    ...
    return (
        ...
        >
            {/* Workspace: ícones + nome da empresa */}
            ...
            {profileOpen ? (
                ...
                >
                    ...
                    />
                    {profileMsg ? <div style={{ marginTop: 8, color: profileMsg.startsWith("✅") ? "#0DAA00" : "crimson" }}>{profileMsg}</div> : null}
                </ModalSimple>
            ) : null}
        </aside>
    );
...
$adminSidebar | Out-File -FilePath "components/AdminSidebar.tsx" -Encoding utf8
Write-Host "Wrote components/AdminSidebar.tsx"

# --------------------------
# 4) app/(admin)/produtos/lista/ListaClient.tsx
# --------------------------
$lista = @'
"use client";
...
export default function ProdutosListaPage() {
    ...
    // modal para cadastrar produto (abre iframe com /produtos)
    ...
    return (
        <main style={{ padding: 24, maxWidth: 1300 }}>
            ...
            <section style={{ marginTop: 16, padding: 16, border: "1px solid #ddd", borderRadius: 14, background: "#fff", boxShadow: "0 8px 20px rgba(32,26,43,0.06)" }}>
                <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap", justifyContent: "space-between" }}>
                    ...
                    {/* Botão Cadastrar produto no topo do card */}
                    <div>
                        <button
...
# we'll append the remaining portion (modal content + submodals + new create modal)
$lista_tail = @'
                {/* CATEGORIA */}
            ...
            {/* MODAL: CADASTRAR PRODUTO (iframe com a página /produtos) */}
            <Modal title="Cadastrar produto" open={createOpen} onClose={() => setCreateOpen(false)}>
                <div style={{ width: "min(860px, 100%)", height: "70vh" }}>
                    <iframe
                        src="/produtos"
...
$lista + $lista_tail | Out-File -FilePath "app/(admin)/produtos/lista/ListaClient.tsx" -Encoding utf8
Write-Host "Wrote app/(admin)/produtos/lista/ListaClient.tsx"

# --------------------------
# Git add / commit
# --------------------------
git add -A
git commit -m "UI: mover Sair pra sidebar; adicionar editar perfil; cadastrar produto abre modal (iframe); fundo cinza e cards com sombra"

Write-Host "Commit criado. Verifique o status e rode npm run dev para testar."
