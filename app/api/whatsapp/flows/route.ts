...
export async function POST(req: NextRequest) {
    ...
    // ─── placeholder para antigo bloco PRODUCTS→CEP_SEARCH (removido, mantém compilação) ──
    if (false as unknown as boolean) {
        return encryptedOk(
            ...
            if (screen === "PAYMENT") {
                ...
                // Carrega sessão (endereço já salvo pelo handler ADDRESS)
                const { data: sessionRow } = await admin
                    .from("chatbot_sessions")
                    .select("id, cart, context, customer_id")
                    .eq("thread_id", threadId)
