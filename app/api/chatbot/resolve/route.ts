...
export async function POST(req: Request) {
    ...
    const { data: bots } = await supabaseAdmin
        .from('chatbots').select('*').eq('company_id', companyId).eq('is_active', true).limit(1);
    ...
    const { data: intents } = await supabaseAdmin
        .from('bot_intents').select('*').eq('company_id', companyId).eq('active', true);
        ...
        for (const it of intents) {
            try {
                ...
                for (const ex of examples) {
                    if (!ex) continue;
                    if (text.toLowerCase().includes(ex.toLowerCase())) { matched = it; break; }
                }
                if (matched) break;
    ...
    if (!threadId) {
        ...
        const { data: newThread } = await supabaseAdmin
            ...
            .select('id')
            .limit(1);
        threadId = Array.isArray(newThread) && newThread.length ? newThread[0].id : (newThread && (newThread as any).id) ?? null;
    }
