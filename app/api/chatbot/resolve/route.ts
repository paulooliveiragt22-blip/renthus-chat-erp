...
export async function POST(req: Request) {
    // requireCompanyAccess já cria/verifica o admin (createAdminClient) e retorna no ctx
    const ctx = await requireCompanyAccess(["owner", "admin", "staff"]);
    if (!ctx.ok) return NextResponse.json({ error: ctx.error }, { status: ctx.status });

    const { companyId, admin: supabaseAdmin } = ctx;
    ...
    if (!threadId) {
        ...
        threadId =
            (Array.isArray(newThread) && newThread.length ? newThread[0].id : (newThread && (newThread as any).id)) ?? null;
    }
