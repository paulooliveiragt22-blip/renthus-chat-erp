...
export default function RelatoriosPage() {
    ...
    const fetchDaily = async (s?: string, e?: string) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                // O endpoint já retorna apenas dias com lançamentos (sem zeros)
                setDaily(j.data ?? []);
            }
        } catch (err: any) {
    ...
    // filtragem por mínimos e colunas visíveis
    const filteredDaily = useMemo(() => {
        const arr = daily ?? [];
        return arr.filter((d) => {
            // já removemos dias completamente zerados no backend; aqui aplicamos mínimos
    ...
    const exportCSV = () => {
        ...
        const rows = filteredDaily.map((r) => {
            ...
            // Escape commas by quoting if necessary (basic)
            return row.map((c) => {
    ...
    return (
        <div style={{ padding: 18 }}>
            ...
            <div style={{ marginTop: 18 }}>
                ...
                {/* Controles de filtro e export */}
                <div style={{ marginTop: 18, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                    <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
                        <label style={{ fontSize: 13 }}><input type="checkbox" checked={showFaturamento} onChange={() => setShowFaturamento(v => !v)} /> Faturamento</label>
                        <label style={{ fontSize: 13 }}><input type="checkbox" checked={showPedidos} onChange={() => setShowPedidos(v => !v)} /> Pedidos</label>
                        <label style={{ fontSize: 13 }}><input type="checkbox" checked={showMensagens} onChange={() => setShowMensagens(v => !v)} /> Mensagens</label>

                        <label style={{ fontSize: 13, marginLeft: 10 }}>Mín. Faturamento: <input type="number" min={0} value={minFaturamento} onChange={(e) => setMinFaturamento(Number(e.target.value) || 0)} style={{ width: 120 }} /></label>
                        <label style={{ fontSize: 13 }}>Mín. Pedidos: <input type="number" min={0} value={minPedidos} onChange={(e) => setMinPedidos(Number(e.target.value) || 0)} style={{ width: 80 }} /></label>
                        <label style={{ fontSize: 13 }}>Mín. Mensagens: <input type="number" min={0} value={minMensagens} onChange={(e) => setMinMensagens(Number(e.target.value) || 0)} style={{ width: 80 }} /></label>
                    </div>

                    <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={exportCSV} style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #ddd", background: "#fff" }}>Exportar CSV</button>
                        <button onClick={exportPDF} style={{ padding: "8px 12px", borderRadius: 8, background: "#3B246B", color: "#fff", border: "none" }}>Exportar PDF (até 90 dias)</button>
                    </div>
                </div>
