...
export default function PlatformDashboard() {
    ...
    const { data: queueHealthRaw, isLoading: isQueueLoading, dataUpdatedAt: queueUpdatedAt } = useQuery({
        queryKey: ["platform", "queue-health", periodMinutes],
        queryFn: () => platformApi.metrics("queue", periodMinutes),
        staleTime: 30_000,
        refetchInterval: autoRefresh ? 30_000 : false,
        refetchIntervalInBackground: true,
    });
    const { data: proMetrics, isLoading: isProMetricsLoading, dataUpdatedAt: proMetricsUpdatedAt } = useQuery({
        queryKey: ["platform", "pro-pipeline-metrics", periodMinutes],
        queryFn: () => platformApi.metrics("pipeline", periodMinutes),
        ...
        <div className="space-y-6">
            ...
            <div className="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
                ...
                <div className="flex flex-wrap gap-3">
                    {[
                        { label: "Ver todas as empresas", href: "/platform/empresas" },
                        { label: "Ver canais WA",         href: "/platform/canais"   },
                        { label: "Ver pedidos",           href: "/platform/pedidos"  },
                        { label: "Segurança / ambiente",   href: "/platform/seguranca" },
                    ].map((a) => (
                        <Link
                            key={a.href}
                            href={a.href}
