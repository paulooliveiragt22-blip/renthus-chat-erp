...
function AdicionarCanalModal({
    ...
    const create = useMutation({
        mutationFn: () => platformApi.createChannel({ company_id: companyId, ...form }),
        onSuccess: () => {
...
function EditarCredenciaisModal({
    ...
    const save = useMutation({
        mutationFn: () => platformApi.updateChannelCredentials(channel.id, form),
        onSuccess: () => {
            toast.success("Credenciais atualizadas!");
            queryClient.invalidateQueries({ queryKey: ["platform", "company"] });
            queryClient.invalidateQueries({ queryKey: ["platform", "channels"] });
            onClose();
        },
        onError: (e: Error) => toast.error(e.message),
    });
...
export default function CompanyDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const { data, isLoading, error, refetch, isFetching } = useQuery({
        ...
        queryFn:  () => platformApi.company(id),
    });
    ...
    const saveComp = useMutation({
        mutationFn: () => platformApi.updateCompany(id, compForm as Record<string, unknown>),
        onSuccess: () => {
    ...
    const saveSub = useMutation({
        mutationFn: () => {
            const sub = (data as any).sub;
            return platformApi.updateSubscription(sub.id, subForm as Record<string, unknown>);
        },
        onSuccess: () => {
                ...
                <button
                    ...
                    onClick={async () => {
                        const reason = window.prompt(
                            "Motivo do acesso suporte (obrigatório, mín. 3 chars):"
                        );
                        ...
                        try {
                            await platformApi.startImpersonation(id, reason.trim());
                            toast.success("Modo suporte ativo (somente leitura)");
                    ...
                    className="flex items-center gap-2 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-900 shadow-sm hover:bg-amber-100 disabled:opacity-50 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-200"
                >
