...
async function downloadImageBestEffort(
    ...
    try {
        ...
        const path = `${companyId}/${productId}/ifood-${Date.now()}.${ext}`;
...
async function upsertItem(
    ...
    if (pErr || !product?.id) {
        // Nome duplicado → tenta atualizar pelo nome
...
export async function syncIfoodCatalogForCompany(
    ...
    try {
        ...
        if (!unSiglaId) {
            throw new Error("Empresa sem sigla comercial UN cadastrada.");
        }
        ...
        for (const item of snapshot.items) {
            try {
                await upsertItem(admin, companyId, item, unSiglaId, counters);
            } catch (err) {
                counters.errors += 1;
                console.warn(
        ...
        await admin
            ...
            .update({
                ...
                last_sync_images: counters.imagesDownloaded,
                last_sync_errors: counters.errors,
                last_error: ok ? null : "Sync parcial com erros",
