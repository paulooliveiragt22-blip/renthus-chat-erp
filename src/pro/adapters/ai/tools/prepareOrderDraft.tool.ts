 ...
 *
 * Groq/OpenAI strict JSON Schema: `.nullable()` sozinho marca a key como **required**
 * (valor pode ser null). Modelos costumam **omitir** campos opcionais → rejeição
 * `missing properties: 'ready_for_confirmation'` e o turno cai em AI_PROVIDER_ERROR + cardápio.
 * `.nullish()` = ausente OU null — alinha com o default em `normalizePrepareDraftAnthropicInput`.
 */
const prepareOrderDraftInputSchema = z.object({
    ...
    address_raw: z.string().nullish().describe("Endereço em uma linha, texto livre do cliente."),
    saved_address_id: z.string().nullish().describe("UUID de um endereço salvo (get_order_hints)."),
    use_saved_address: z.unknown().nullish().describe("true para usar o endereço padrão salvo do cliente."),
    payment_method: z.string().nullish().describe("pix | cash | card — só se o cliente já informou."),
    change_for: z.unknown().nullish().describe("Valor do troco em reais — só se o cliente pediu."),
    ready_for_confirmation: z
        .unknown()
        .nullish()
        .describe("true só quando o cliente pediu para confirmar/fechar o pedido."),
    order_notes: z
        .string()
        .nullish()
        .describe(
