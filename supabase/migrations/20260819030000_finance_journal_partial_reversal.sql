-- Fase D (journal-first): estorno parcial/total por linhas do lançamento.

-- ─── 1. Remaining por linha do journal original ─────────────────────────────
create or replace function public.fn_fin_journal_line_remaining(
  p_journal_id uuid,
  p_account_code text,
  p_direction text
)
...
as $$
  select round(
    coalesce(orig.amt, 0) - coalesce(rev.amt, 0),
    2
  )
  from (
  ...
  (
    select sum(l.amount) as amt
      ...
      join public.chart_of_accounts a on a.id = l.account_id
     where j.reverses_id = p_journal_id
       and j.status = 'posted'
       and l.direction = case when p_direction = 'credit' then 'debit' else 'credit' end
       and a.code = p_account_code
  ) rev;
$$;

revoke all on function public.fn_fin_journal_line_remaining(uuid, text, text) from public;
grant execute on function public.fn_fin_journal_line_remaining(uuid, text, text) to service_role;

-- ─── 2. Detalhe do journal (linhas + remaining) ─────────────────────────────
...
-- ─── 3. Estorno parcial (linhas do lançamento) ─────────────────────────────
...
begin
  ...
  -- Conta líquida (caixa / AR) no journal original
  if exists (
    select 1
      from public.finance_journal_lines l
      join public.chart_of_accounts a on a.id = l.account_id
  ...
  v_new := public.fn_fin_post_journal(
    ...
    v_j.origin, v_j.payment_method, now(), now(), null, p_reason, 'Estorno parcial', p_journal_id,
    v_out
  );

  -- Marca reversed quando não há remaining em nenhuma linha original
  if not exists (
    select 1
      from public.finance_journal_lines l
      join public.chart_of_accounts a on a.id = l.account_id
...
-- ─── 4. Estorno total — só o que ainda tem remaining ────────────────────────
