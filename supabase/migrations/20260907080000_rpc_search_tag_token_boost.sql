-- Apelidos em tags (ex.: "caixinha") são muito usados no WhatsApp.
-- Boost por TOKEN de tag (não frase inteira) + bônus extra quando a marca
-- do pedido também casa no product_name. Não reduz score de name/descrição:
-- é aditivo sobre o greatest(...) existente.
...
BEGIN
  ...
  -- Tokens do pedido + singular simples (caixinhas→caixinha). Stopwords curtas fora.
  SELECT coalesce(array_agg(DISTINCT tok), ARRAY[]::text[])
  INTO v_tokens
  FROM (
    SELECT t AS tok
    FROM unnest(regexp_split_to_array(v_q_norm, '[^a-z0-9]+')) AS t
    WHERE length(t) >= 4
      AND t NOT IN (
        'quero', 'manda', 'pedir', 'pedido', 'para', 'pra', 'com',
        'uma', 'umas', 'uns', 'dois', 'duas', 'tres', 'quatro', 'cinco',
        'caixa', 'caixas', 'unidade', 'unidades', 'fardo', 'fardos',
        'pacote', 'pacotes', 'pack', 'packs'
      )
    UNION ALL
    SELECT left(t, length(t) - 1)
    FROM unnest(regexp_split_to_array(v_q_norm, '[^a-z0-9]+')) AS t
    WHERE length(t) > 5
      AND right(t, 1) = 's'
      AND right(t, 2) <> 'ss'
      AND t NOT IN ('quero', 'manda', 'caixa', 'caixas', 'unidade', 'unidades')
  ) s
  WHERE length(tok) >= 4;
    ...
    SELECT
      ...
      least(
        ...
        + CASE
            ...
            ELSE 0
          END
        -- Apelido (tag) + marca: aditivo. Tag sozinha não compete com name no greatest
        -- (evita Heineken "caixinha" empatar com Original quando o pedido cita a marca).
        + CASE
            WHEN EXISTS (
              SELECT 1
              FROM unnest(v_tokens) AS tok
              WHERE (
                lower(unaccent(coalesce(v.tags, ''))) LIKE '%' || tok || '%'
                OR lower(unaccent(coalesce(v.tags_auto, ''))) LIKE '%' || tok || '%'
              )
              AND lower(unaccent(coalesce(v.product_name, ''))) NOT LIKE '%' || tok || '%'
              AND lower(unaccent(coalesce(v.descricao, ''))) NOT LIKE '%' || tok || '%'
            )
            AND EXISTS (
              SELECT 1
              FROM unnest(v_tokens) AS brand
              WHERE lower(unaccent(coalesce(v.product_name, ''))) LIKE '%' || brand || '%'
            )
            THEN 0.48
            WHEN EXISTS (
              SELECT 1
            ...
            -- Marca no pedido que NÃO é desta linha → boost fraco (não rouba o top)
            AND EXISTS (
              SELECT 1
              FROM unnest(v_tokens) AS brand
              WHERE length(brand) >= 4
                AND lower(unaccent(coalesce(v.product_name, ''))) NOT LIKE '%' || brand || '%'
                AND lower(unaccent(coalesce(v.tags, ''))) NOT LIKE '%' || brand || '%'
                AND lower(unaccent(coalesce(v.tags_auto, ''))) NOT LIKE '%' || brand || '%'
            )
            THEN 0.06
            WHEN EXISTS (
              SELECT 1
...
COMMENT ON FUNCTION public.rpc_search_chat_produtos(uuid, text, integer) IS
  'Busca fuzzy + boost de apelido em tags (token). Combo tag+marca tem peso alto; name/descrição intactos.';
