-- EX3: rpc_fulfill_obligation — claim invoice + activate subscription numa transação.
-- ai_pack e provision de auth user permanecem no Node (pós-claim).
...
begin
  ...
  v_kind := case when v_is_setup then 'setup' else 'invoice' end;
  ...
  -- +1 mês civil (equivale a date-fns addMonths)
...
comment on function public.rpc_fulfill_obligation(text, text, text, text) is
  'EX3: claim invoice pending→paid + activate pagarme_subscriptions + companies.is_active (atomic).';
