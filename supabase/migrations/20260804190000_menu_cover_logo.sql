-- Capa do restaurante no cardápio web (estilo Facebook) + RPC com cover_url.

ALTER TABLE public.company_menu_profile
    ADD COLUMN IF NOT EXISTS cover_url text NULL;

COMMENT ON COLUMN public.company_menu_profile.cover_url IS
    'URL da capa do cardápio público (banner estilo Facebook).';
COMMENT ON COLUMN public.company_menu_profile.logo_url IS
    'URL da foto de perfil (avatar circular) no cardápio público.';
