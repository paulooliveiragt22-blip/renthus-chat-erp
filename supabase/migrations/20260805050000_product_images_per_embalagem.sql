-- ============================================================
-- Foto por embalagem/sigla (produto_embalagem_id), não só por volume.
-- Fallback de exibição: item → volume (legado) → produto.
-- ============================================================

ALTER TABLE public.product_images
  ADD COLUMN IF NOT EXISTS produto_embalagem_id uuid NULL
    REFERENCES public.produto_embalagens(id) ON DELETE CASCADE;

CREATE INDEX IF NOT EXISTS idx_product_images_embalagem
  ON public.product_images (produto_embalagem_id)
  WHERE produto_embalagem_id IS NOT NULL;

COMMENT ON COLUMN public.product_images.produto_embalagem_id IS
  'Escopo da foto por item/sigla (UN, CX…). Preferido sobre product_volume_id.';

-- Primary: um por escopo de embalagem; se sem embalagem, um por volume/produto
...
BEGIN
  IF NEW.is_primary THEN
    IF NEW.produto_embalagem_id IS NOT NULL THEN
      UPDATE public.product_images
      SET is_primary = false
      WHERE product_id = NEW.product_id
        AND id <> NEW.id
        AND is_primary = true
        AND produto_embalagem_id IS NOT DISTINCT FROM NEW.produto_embalagem_id;
    ELSE
      UPDATE public.product_images
      SET is_primary = false
      WHERE product_id = NEW.product_id
        AND id <> NEW.id
        AND is_primary = true
        AND produto_embalagem_id IS NULL
...
-- ── view_chat_produtos: imagem item → volume → produto ───────────────────────
