...
|---|------|-------------------|-----|--------|
| O1.1 | Confirmar URL webhook Pagar.me → `https://…/api/billing/webhook` (prod) | Painel Pagar.me + `docs/SMOKE_BILLING_PAGARME_SANDBOX.md` | URL e eventos `order.paid` / `charge.paid` ativos | [~] 2026-09-03 — POST chega (não mais 401). **500** por `replaceAll(/^sha256=/i)` sem flag `g` em `verifyWebhookSignature` — corrigido; **redeploy + reenviar** para fechar O1.3 |
