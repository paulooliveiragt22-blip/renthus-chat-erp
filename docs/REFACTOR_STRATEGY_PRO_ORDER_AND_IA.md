...
**Próximo na sequência sugerida (§0):** fechar critério §6 “documentação” (`CHATBOT_PROD.md` alinhado — feito neste passo para V2); em código, **R2** mínimo em `inboundPipeline.ts` (comentário/guard explícito quando V2 active + sucesso já tratado em `processMessage`) ou **R4** (quota leitura catálogo) quando houver evidência; manter [`SMOKE_RUNBOOK_PRO_PIPELINE_V2.md`](./SMOKE_RUNBOOK_PRO_PIPELINE_V2.md) em sincronia a cada mudança de confirmação/telemetria.
