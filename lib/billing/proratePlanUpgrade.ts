 ...
 * Delta = (destino − atual) × dias_restantes / ciclo.
 *
 * `proratePlanUpgradeCents` é o espelho puro (spec de teste do caso mensal). A
 * fonte canônica em runtime é a RPC do banco `rpc_quote_plan_upgrade`, que é
 * período-aware (anual = delta anual / 365) e chama `fn_billing_prorate_cents`
 * internamente (ADR-0006 D12 / governanca §2).
 */
